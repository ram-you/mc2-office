<template> 
    <v-card class="pa-4 ma-4">
      <v-layout row wrap style="max-width:800px;">
        <v-flex xs6>
        ...
        <v-flex xs6>
          <v-select :items="items" v-model="e3" :error-messages="['Please select an option']" label="Select" item-value="text"></v-select>
        </v-flex>
        <v-flex xs6>
          <v-subheader>Disabled</v-subheader>
        </v-flex>
        <v-flex xs6>
          <v-select :items="items" v-model="e4" label="Select" disabled></v-select>
        </v-flex>
      </v-layout>
    </v-card> 
</template>
